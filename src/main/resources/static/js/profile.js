/**
     [회원 프로필 페이지]
  (1) 회원 프로필 페이지 팔로우, 팔로우취소
  (2) 팔로잉 정보 모달 보기
  (2-1) 팔로워 정보 모달 보기
  (3) 팔로잉 정보 모달에서 팔로우, 팔로우취소
  (3-1) 팔로워 정보 모달에서 팔로우, 팔로우취소
  (4) 회원 프로필 사진 변경
  (5) 회원 정보 메뉴 열기 닫기
  (6) 회원 정보(회원정보 변경, 비밀번호 변경, 로그아웃, 닫기) 모달
  (7) 회원 프로필 이미지 메뉴(사진업로드, 취소) 모달
  (8) 팔로우 정보 모달 닫기
 */
...
function getFollowingModalItem(u) {
	...
	let item=`<div class="follow__item" id="followingModalItem-${u.userId}">
...
function getFollowerModalItem(u) {
	...
	let item=`<div class="follow__item" id="followerModalItem-${u.userId}">
	<div class="follow__img">
			<img src="/upload/${u.profileImageUrl}" onerror="this.src='/images/person.jpg'"/>
		</div>
		<div class="follow__text">
...
function profileImageUpload(principalId) {
	...
	$("#userProfileImageInput").on("change", (e) => {
		...
		$.ajax({
			...
			reader.readAsDataURL(f); // reader.onload 실행됨.
